...
def make_msa_features(msas: Sequence[parsers.Msa]) -> FeatureDict:
    ...
    k = int(ln*0.4)
    idx = random.sample(list(range(ln)), k)
    idx_d = [x for x in list(range(ln)) if (x not in idx)]
    q = int(len(idx_d)/2)
    idx_e = [idx_d[a] for a in range(q-1, q+2)] 

    p = lambda x: ''.join([x[a] for a in idx_e])

    while (m < 2000) and (n<5000):
        for msa_index, msa in enumerate(msas):
            ...
            for sequence_index, sequence in enumerate(msa.sequences):
                ...
                if p(sequence) not in seen_sequences:
                    seen_sequences[p(sequence)] = set()
                if sequence in seen_sequences[p(sequence)]:
                    continue
                ...
                for x in idx_d:
                    if string_list[x] == '-':
                        string_list[x] = query[x]
                sequence = ''.join(string_list)
                sequence = mutate(sequence, idx_d, ln) 
                ...
                seen_sequences[p(sequence)].add(sequence)              
                int_msa.append(
...
def process_dssp(sequence: str):
    dssp_one_hot = residue_constants.sequence_to_onehot(
        sequence=sequence,
        mapping=residue_constants.psstype_order_with_x,
        map_unknown_to_x=True,
    )
    return dssp_one_hot
...
class DataPipeline:
    ...
    def _process_msa_feats(
        ...
        #idx = [ax for ax in range(len(input_sequence)) if input_sequence[ax] == 'X']
        num_res = len(input_sequence)
    ...
    def process_fasta(
        ...
        with open(fasta_path) as f:
            fasta_str = f.read()
        if len([x for x in fasta_str.split('\n') if x.strip() != '']) == 2:
            input_seqs, input_descs = parsers.parse_fasta(fasta_str)
        else:
            dssp = fasta_str.split('\n')[2].strip()
        ...
        if dssp is not None:
            sequence_features["psstype"] = process_dssp(dssp)
        else:
            print("Error: PSSP is not provided")
            sys.exit()
    ...
    def process_mmcif(
        ...
        input_sequence = self.mutate(input_sequence)
        ...
        if seqemb_mode:
            ...
            msa_features = self._process_msa_feats(alignment_dir, input_sequence, alignment_index)
        #features =  {**mmcif_feats, **template_features, **msa_features, **sequence_embedding_features}
        """
        features =  {**mmcif_feats, **msa_features, **sequence_embedding_features}

        del mmcif_feats
        del msa_features
        del sequence_embedding_features
        return features
    ...
    def mutate(self, seq):
        ...
        num_res = len(seq)
        k = int(num_res*0.4)
        idx = random.sample(list(range(num_res)), k)
        seq = ''.join([a if x not in idx else 'X' for x, a in enumerate(seq)])
        #for _ in range(k):
        #    x1 = random.randint(0, num_res-1)
            #x2 = random.randint(x1, num_res-1)
            #if seq[x2] != seq[x1]:
            #    seq = seq[:x1] + seq[x2] + seq[x1+1: x2] + seq[x1] + seq[x2+1:]
        #    seq = seq[:x1] + 'X' + seq[x1+1:]
        return seq
