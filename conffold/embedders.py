...
class InputEmbedder(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.layernorm1 = LayerNorm(c_m)
        self.layernorm2 = LayerNorm(c_m)
    ...
    def forward(
        ...
        msa_emb = self.layernorm1(msa_emb)
        tf_m = self.layernorm2(tf_m)
...
class InputEmbedder1(nn.Module):
    """
    Embeds a subset of the input features.
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.linear_tf_z_i = Linear(tf_dim, c_z1)
        self.linear_tf_z_j = Linear(tf_dim, c_z1)
        self.linear_tf_m = Linear(tf_dim, c_m1)

        self.linear_tf_z_i1 = Linear(tf_dim1, c_z1)
        self.linear_tf_z_j1 = Linear(tf_dim1, c_z1)
        self.linear_tf_m1 = Linear(tf_dim1, c_m1)

        self.linear_msa_m = Linear(msa_dim, c_m1)
    ...
    def forward(
        ...
        c = len(tf.shape)
        if c > 2:
            tf1 = torch.cat((tf[:, :, :1], tf[:, :, 22:]), c-1)
            tf = self.dropout1(tf[:, :, 1:22])
        else:
            tf1 = torch.cat((tf[:, :1], tf[:, 22:]), c-1)
            tf = self.dropout1(tf[:, 1:22])

        tf_emb_i = torch.cat((self.linear_tf_z_i(tf), self.linear_tf_z_i1(tf1)), c-1)
        tf_emb_j = torch.cat((self.linear_tf_z_j(tf), self.linear_tf_z_j1(tf1)), c-1)

        tf_m = self.linear_tf_m(tf)
        tf_m1 = self.linear_tf_m1(tf1)
        ...
        if c > 2:
            msa = self.dropout2(msa)

        msa_emb = self.linear_msa_m(msa) + tf_m
        msa_emb = torch.cat((msa_emb,tf_m1), c)
        return msa_emb, pair_emb
...
class ExtraMSAEmbedder(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.dropout = nn.Dropout(p=0.5)
        self.layernorm = LayerNorm(c_out)
