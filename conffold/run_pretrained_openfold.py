...
from openfold.utils.script_utils import (load_models_from_command_line, parse_fasta, run_model,
                                         prep_output, relax_protein, parse_dssp)
...
def generate_feature_dict(
    ...
    if "multimer" in args.config_preset:
        ...
        with open(tmp_fasta_path, "w") as fp:
            fp.write(
                '\n'.join([f">{tag}\n{seq}\n{dssp}" for tag, seq, dssp in zip(tags, seqs, dssps)])
            )
...
def main(args):
    ...
    for fasta_file in list_files_with_extensions(args.fasta_dir, (".fasta", ".fa")):
        ...
        if args.pssp_fasta == False:
            dssp_path = os.path.join(args.fasta_dir, fasta_file.replace(".fasta", ".dssp"))
            with open(dssp_path, "r") as fp:
                data = fp.read()
                dssp = parse_dssp(data)
        else:
            dssp_path = os.path.join(args.fasta_dir, fasta_file.replace(".fasta", ".pssp"))
            with open(dssp_path, "r") as fp:
                data = fp.read()
                dssp_tags, dssp = parse_fasta(data)
        ...
        if args.pssp_fasta == False:
            ...
            for x in range(len(dssp)):
                tag_list.append((tag, [tags[0]], dssp_tags[x]))
                seq_list.append(seqs)
                dssp_list.append([dssp[x]])
    
    seq_sort_fn = lambda target: sum([len(s) for s in target[1]])
    sorted_targets = sorted(zip(tag_list, seq_list, dssp_list), key=seq_sort_fn)
    feature_dicts = {}
    ...
    for model, output_directory in model_generator:
        ...
        for (tag, tags, m_tag), seqs, dssp in sorted_targets[:]:
            output_name = f'{tags[0]}_{m_tag}_{args.config_preset}'
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--skip_relaxation", action="store_true", default=True,
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--pssp_fasta", action="store_true", default=False,
        help="Whether to use the FASTA file containing PSSP.",
    )
